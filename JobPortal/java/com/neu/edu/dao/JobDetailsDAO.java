...
private JobDetails jobdescription;
	public List<JobDetails> searchJob(String jobDetail){
		...
		try{
			...
			Criteria criteria = getSession().createCriteria(JobDetails.class);
			criteria.add(Restrictions.disjunction()
					.add(Restrictions.like("jobTitle", jobDetail,MatchMode.ANYWHERE))
			.add(Restrictions.like("company", jobDetail,MatchMode.ANYWHERE))
			.add(Restrictions.like("location", jobDetail,MatchMode.ANYWHERE))
			.add(Restrictions.like("employmentType", jobDetail,MatchMode.ANYWHERE)));
			job =  criteria.list();
			commit();
...
/*	public void addJob(JobApplication jobapply){
	...
	public void applyJob(int jobID,String companyName, float workExp, User user,String status){
	...
	public List<JobApplication> fetchApplicants(String jobId){
		...
		try{
			...
			cr.add(Restrictions.eq("jobApplication.status","pending"));
	...
	public List<JobDetails> appliedJobs(User user){
		...
		try{
			...
			Query query = getSession().createSQLQuery("select  distinct job.JobID,job.company,applied.status, job.description, "
